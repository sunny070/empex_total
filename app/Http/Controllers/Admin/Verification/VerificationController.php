...
class VerificationController extends Controller
{
    ...
    public function postVerify($id, $status, $router = null, Request $request)
    {
        ...
        // added
        // dd($status);
        $basicInfo->verifier_user_id = Auth()->user()->id;
        $basicInfo->verified_date = Carbon::now()->setTimezone('Asia/Kolkata')->format('y-m-d');
        if ($status == 'Approved') {
              $basicInfo->approver_user_id = Auth()->user()->id;
              $basicInfo->approved_date = Carbon::now()->setTimezone('Asia/Kolkata')->format('y-m-d');
       }
        // if ($status == 'Approved') {
        // }
        // dd($status);

					...
					<table class="min-w-full divide-y divide-gray-200">
						<thead>
							<tr>
								...
								<th scope="col"
									class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wide">
									Action
								</th>
							</tr>
						</thead>
						<tbody class="bg-white divide-y divide-gray-200">
							...
							<tr class="hover:bg-gray-50">
								<td class="px-6 py-4 whitespace-nowrap">
									<img src="{{ asset('/storage/'.$emp->avatar) }}" alt="avatar"
								...
								<td class="px-6 py-4 whitespace-nowrap">
									<input
										class="w-5 h-5 rounded-full text-empex-green focus:outline-none cursor-pointer focus:ring-0"
										type="checkbox" id="place{{ $emp->id }}"
										wire:click='changeIsPlaced({{ $emp->id }})' {{ $emp->is_placed == 1 ? 'checked'
									: '' }}>
									<label class="form-check-label inline-block ml-1 cursor-pointer"
										for="place{{ $emp->id }}">
